...
function CreateFarmingPool() {
  ...
  return (address ? <div className='bg-cream p-4 rounded-xl shadow-md' >
        <div>
          <div>
            <div className='w-full flex items-center justify-between'>
            <Grid item xs={6}>
              <p className='font-semibold w-1/2 text-md'>Staked Liquidity</p>
            </Grid>
            <IntegrationTrigger
                title={"Select a Liquidity pool"}
                type="pool"
                selectToken={handleSelectToken}
                customTokens={Array.from(liquidityPools.entries()).map((value : any, i : any) => {
                  return {
                    "address": value[0],
              ...
              >
                {Object.keys(stakedToken).length === 0 ? (<div 
                  className='!w-full cursor-pointer bg-[#D7009A] text-white flex justify-between font-bold py-1 px-2 rounded-2xl'> 
                    Select a liquidity pool <ChevronDown className='mt-[1px]' /></div>) : (<div 
                    className='!w-full right-0 float-end cursor-pointer bg-white flex justify-between font-bold py-1 px-2 rounded-2xl'>
                      <Image src={stakedToken.logo ? stakedToken.logo : network.image} alt="eth" width={25} height={20} className="mr-1" /> 
                      {stakedToken.symbol}<ChevronDown className='mt-[1px]' />
                      </div>)}
              </div>
              </IntegrationTrigger>
            </div>
                ...
                {Object.keys(rewardToken).length === 0 ? (<div 
                  className='!w-36 cursor-pointer bg-[#D7009A] text-white flex justify-between font-bold py-1 px-2 rounded-2xl'> 
                    ...
                    className='!w-full !max-w-[118px] right-0 float-end cursor-pointer bg-white flex justify-between font-bold py-1 px-2 rounded-2xl'>
                      <Image src={rewardToken.logo ? rewardToken.logo : network.image} alt="eth" width={25} height={20} className="mr-1" /> 
                      {rewardToken.symbol}<ChevronDown className='mt-[1px]' />
                      </div>)}
              </div>
              </IntegrationTrigger>
            </div>
            <div className='w-full mt-6 flex items-center justify-between'>
                ...
                <input type='number' id="reward_per_block"  value={rewardPerBlock == 0 ? '' : rewardPerBlock} 
            ...
            <div className='w-full mt-6 flex items-center justify-between'>
             ...
             <div className='w-full mt-6 flex items-center justify-between'>
                 ...
                 <input type='number' id="end_block"  value={endBlock == 0 ? '' : endBlock} 
                 onChange={handleChange} placeholder='0' 
                 className='bg-none w-1/2 text-xl font-bold placeholder:text-slate-500 placeholder:font-bold placeholder:text-3xl bg-white shadow-sm p-2 rounded-lg outline-none border-none' />
             </div>
            ...
            <Grid item xs={4}></Grid>
            <Grid item xs={4}>
              <Button className='w-full mt-3 bg-white' disabled={startBlock >= endBlock || !tokensSelected}  fullWidth onClick={() => handleCreate()}>
                {loading ? <CircularProgress sx={{ color: '#D7009A' }} /> : "Create"}
              </Button>
            </Grid>
            <Grid item xs={4}></Grid>
          </div>
        </div>
      </div> : <p className="h-[100px] font-semibold text-[#D7009A] justify-center items-center flex bg-cream rounded-xl mt-2  w-full">Please connect to a wallet to create staking pool</p>
    );
}
