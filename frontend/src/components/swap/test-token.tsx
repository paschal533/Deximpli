...
function TestTokenModal({ selectToken }: { selectToken: any }) {
  ...
  const placeholders = ["Search name of token", "Paste address of token"];
            ...
            tokens.length > 0 ? (
              tokenList.map((item: any, index: any) => (
                <DialogClose key={index} className="flex-col w-full">
                  ...
                  >
                    <Image
                      src={item.logo ? item.logo : network.image}
                      alt="eth"
                      width={30}
                      height={30}
                    />
