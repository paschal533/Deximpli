...
const Withdraw = ({ tokenAddress } : { tokenAddress : any}) => {
  ...
  const getShareBalance = async (assetPool : any, tokenAddress : any) => {
    try {
      ...
      const shareContract = new ethers.Contract(pool.shareToken, ERC20ABI, signer);
    ...
    <Grid item>
      ...
      <div>
          ...
          <div className='w-full p-2 mt-2 flex-col items-center'>
                 ...
                 onChange={handleChange} placeholder={`enter withdraw token (${token.symbol}) amount`} 
        ...
        <p className='p-2 font-semibold'>Withdrawable balance of {token.symbol}:</p>
        <div className='space-x-1 justify-center items-center'>
        <span className='font-medium'>{depositBalance}</span>
        <div className="text-xs cursor-pointer float-end font-semibold text-[#D7009A]" onClick={() => setAmount(depositBalance)} >Max</div>
        </div>
        </div>
        <Grid item xs={4}></Grid>
        <div className='w-full flex space-x-2 p-2'>
          <Button className='!bg-cream w-1/2 text-[#D7009A] font-semibold rounded-lg' disabled={amount <= 0 || amount > depositBalance} fullWidth onClick={handleWithdraw} >
          {loading ? <CircularProgress sx={{ color: '#D7009A' }} /> : "Withdraw"}
          </Button>
        </div>
        <Grid item xs={4}></Grid>
