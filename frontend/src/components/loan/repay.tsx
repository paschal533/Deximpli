...
const Repay = ({ tokenAddress } : { tokenAddress : any}) => {
  ...
  const getMaxRepayAmount = useCallback(async (tokenObject : any) => {
    try {
    ...
    <Grid item>
      ...
      <div>
          ...
          <div className='w-full p-2 mt-2 flex-col items-center'>
                 ...
                 onChange={handleChange} placeholder={`enter repay token (${token.symbol}) amount`} 
        ...
        <p className='p-2 font-semibold'>Maximum Repayment Amount {token.symbol}:</p>
        <div className='space-x-1 justify-center items-center'>
        <span className='font-medium'>{maxRepayAmount}</span>
        <div className="text-xs cursor-pointer float-end font-semibold text-[#D7009A]" onClick={() => setAmount(maxRepayAmount)} >Max</div>
        </div>
        </div>
        <Grid item xs={4}></Grid>
        <div className='w-full flex space-x-2 p-2'>
          ...
          <Button className='!bg-cream w-1/2 text-[#D7009A] font-semibold rounded-lg'  disabled={amount <= 0 || allow < amount || amount > maxRepayAmount} fullWidth onClick={handleRepay}>
          {allow >= amount && loading ? <CircularProgress sx={{ color: '#D7009A' }} /> : "Repay"}
          </Button>
        </div>
        <Grid item xs={4}></Grid>
