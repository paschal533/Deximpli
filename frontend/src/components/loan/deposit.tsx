...
const Deposit = ({ tokenAddress }: { tokenAddress: any }) => {
  ...
  const getBalance = useCallback(
    async (tokenObject: any) => {
      try {
        ...
        setBalance(
          Number(ethers.utils.formatUnits(_balance, tokenObject.decimals)),
        );
      } catch (error) {
  ...
  const loadDepositInfo = useCallback(
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      const tx = await assetPool.deposit(
        token.address,
  ...
  if (!address) {
    ...
    return (
      <p className="h-[50px] text-center !font-medium !mt-2 px-4 py-2 rounded-xl  w-full">
        <CircularProgress sx={{ color: "#D7009A", height: "10px" }} />
      </p>
    );
  }
        ...
        <div>
          <div className="w-full p-2 mt-2 flex-col items-center">
            ...
            <input
              ...
              placeholder={`enter deposit token (${token.symbol}) amount`}
          ...
          <div className="w-full flex space-x-2 p-2">
            ...
            >
              {allow < amount && loading ? (
                <CircularProgress sx={{ color: "#D7009A", height: 2 }} />
              ) : (
                "Approve"
              )}
            </Button>
            ...
            <Button
              className="!bg-cream w-1/2 text-[#D7009A] font-semibold rounded-lg"
              disabled={amount <= 0 || allow < amount || amount > balance}
              fullWidth
