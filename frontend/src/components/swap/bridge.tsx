...
function Bridge() {
  const { address, loading, networkselectedA, setNetworkSelectedA, network, signer, currentChainId } = useContext(SwapContext)
 ...
 const selectNetworkB = (network : any) => {
  if (network.name === networkselectedA.name) {
    toast.error('The selected networkss are identical, please select another network!');
  }else{
...
const Bridge = async () => {
    try {
      if(networkselectedB && amount > 0 && token && networkselectedA){
        const contract = new ethers.Contract(networkselectedA.address, CCIPTokenTransferABI.abi, signer);
        const _allowAmount = ethers.utils.parseUnits(amount.toString(), 18);
        let chainSelected = networkselectedB?.name == "OPTIMISM" ? "5224473277236331295" : networkselectedB?.name == "CELO" ? "3552045678561919002" : networkselectedB?.name == "BASE" ? "10344971235874465080" : "829525985033418733"
        let valueAmount = Number(_allowAmount.toString())
        const tokenName = token.name
        const tokenAddress = tokenName == "CCIP-BnM" ? networkselectedA.CCIP_BnM : networkselectedA.CCIP_LnM;
    ...
    <div className='md:w-[500px] md:p-0 p-4 w-[90vw]'>
        <section className='h-[100px] bg-cream rounded-xl px-4 py-2  w-full'>
          <IntegrationTrigger
              ...
              >
                 <div className='w-[170px] cursor-pointer bg-white flex justify-between font-bold py-1 px-2 space-x-2 rounded-2xl'><Image className='mr-1' src={network.image} alt="eth" width={25} height={20} /> {token.name} <ChevronDown className='mt-[1px]' /></div>
              </IntegrationTrigger>
          <div className='flex justify-between mt-2 w-full'>
            <input type='number' value={amount} onChange={(e: any) => setAmount(e.target.value)} placeholder='0' className='bg-none text-3xl w-1/2 font-bold placeholder:text-slate-500 placeholder:font-bold placeholder:text-3xl bg-transparent outline-none border-none' />
            <IntegrationTrigger
              ...
              >
                 <div className=' cursor-pointer bg-white flex justify-between font-bold py-1 px-2 space-x-2 rounded-2xl'>
                  <Image className='mr-1' src={networkselectedA?.image} alt="eth" width={25} height={20} /> 
        ...
        <div className='text-md h-4 font-semibold text-gray-500'></div>
          <div className='flex mb justify-between mt-2 w-full'>
            <input type='number' value={amount} onChange={(e: any) => setAmount(e.target.value)} placeholder='0' className='bg-none w-1/2 text-3xl font-bold placeholder:text-slate-500 placeholder:font-bold placeholder:text-3xl bg-transparent outline-none border-none' />
            <IntegrationTrigger
                title={"Select a Network"}
                type='network'
                selectToken={selectNetworkB}
              >
                {networkselectedB.image == null ? <div className=' cursor-pointer bg-[#D7009A] text-white flex justify-between font-bold py-1 px-2 rounded-2xl'>
                  Select network <ChevronDown className='mt-[1px]' />
                 </div> : <div className=' cursor-pointer bg-white flex justify-between font-bold py-1 px-2 rounded-2xl'>
                 <Image className='mr-1' src={networkselectedB?.image} alt="eth" width={25} height={20} /> {networkselectedB?.name} <ChevronDown className='mt-[1px]' />
                 </div> }
              </IntegrationTrigger>
          </div>
        </section>
        <section>
            {!address ? <ConnectBTN /> : <Button disabled={!networkselectedB || !amount || !token} onClick={Bridge} className="h-[50px] font-semibold text-[#D7009A] mt-2 justify-center items-center flex !bg-cream rounded-xl  w-full" fullWidth>
