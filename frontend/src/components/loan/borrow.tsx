...
const Borrow = ({ tokenAddress }: { tokenAddress: any }) => {
  ...
  const getBorrowableQuota = useCallback(
    async (tokenObject: any) => {
      try {
        ...
        let _quota = Number(
          userInfo.totalCollateralValue
            .sub(userInfo.totalBorrowedValue)
            .div(tokenPrice),
        );
  ...
  const loadBorrowInfo = useCallback(
  ...
  const handleBorrow = async () => {
    ...
    try {
      ...
      const tx = await assetPool.borrow(
        token.address,
        ...
        <div>
          <div className="w-full p-2 mt-2 flex-col items-center">
            ...
            <input
              ...
              placeholder={`enter borrowed token (${token.symbol}) amount`}
          ...
          <div className="w-full flex justify-between pr-2 items-center">
            <p className="p-2 font-semibold">
              Borrowable Quota of {token.symbol}:
            </p>
            <div className="space-x-1 justify-center items-center">
              ...
              <div
                ...
                onClick={() => setAmount(borrowableQuota)}
              >
                Max
              </div>
            </div>
          ...
          <div className="w-full flex space-x-2 p-2">
            <Button
              ...
              disabled={amount <= 0 || amount > borrowableQuota}
              fullWidth
