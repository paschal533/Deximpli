...
const RemoveLiquidity = ({ liquidityPair }: { liquidityPair: any }) => {
  ...
  const getTotalSupply = useCallback(async () => {
    try {
      ...
      toast.error(
        getErrorMessage(error, "Cannot get total supply of token pair!"),
      );
      console.error(error);
    }
  }, [signer, pair]);
  ...
  const getAllowance = useCallback(async () => {
    try {
      const tokenPair = new ethers.Contract(pair, TokenPairABI, signer);
      const _allowAmount = await tokenPair.allowance(
        address,
      ...
      toast.error(
        getErrorMessage(error, "Cannot get allowance of token pair!"),
      );
      console.error(error);
    }
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const tx = await tokenPair.approve(
        SuppotedAMMRouterContractAddress(provider?._network.chainId),
        _allowAmount,
      );
      await tx.wait();
  ...
  const handleRemoveLiquidity = async () => {
    ...
    try {
      ...
      if (isETH(tokenA, provider)) {
        tx = await ammRouter.removeLiquidityETH(
          tokenB.address,
        ...
        tx = await ammRouter.removeLiquidity(
          tokenA.address,
          tokenB.address,
          ethers.utils.parseUnits(toString(amount)),
          0,
  ...
  return address ? (
    pair ? (
      <>
        ...
        <Box width="100%">
          <Typography>
            Removal Percentage: {amountPercent.toFixed(2)} %
          </Typography>
          <Slider
            value={amountPercent}
            onChange={(e, value: any) => setAmount((balance * value) / 100)}
          />
        ...
        <Grid container justifyContent="space-between" alignItems="center">
          ...
          <Grid item>
            <Typography>
              {((reserveA * amount) / totalSupply).toFixed(2)}
            </Typography>
          </Grid>
        </Grid>
        <Grid container justifyContent="space-between" alignItems="center">
          ...
          <Grid item>
            <Typography>
              {((reserveB * amount) / totalSupply).toFixed(2)}
            </Typography>
          </Grid>
        </Grid>
      ...
      <h1 className="h-[50px] font-semibold text-[#D7009A] justify-center items-center flex bg-cream rounded-xl mt-2  w-full">
        No pair specified!
      </h1>
    )
  ) : (
