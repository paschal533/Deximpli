...
const Deposit = ({ poolAddress }: { poolAddress: any }) => {
  ...
  const getBalance = useCallback(async () => {
    ...
    try {
      ...
      setBalance(
        Number(ethers.utils.formatUnits(_balance, stakedToken.decimals)),
      );
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      const tx = await stakingPool.deposit(
        ...
        <div>
          <Grid item>
            <Grid container columnGap={12} className="mb-4">
              <Grid item>
                <Typography sx={{ mt: 1 }}>
                  Deposit Staked Token ({stakedToken.symbol})
                </Typography>
              </Grid>
            </Grid>
            <Divider />
            <div>
              <div className="w-full p-2 mt-2 flex-col items-center">
                ...
                <input
                  ...
                  placeholder={`enter staked token (${stakedToken.symbol}) amount`}
