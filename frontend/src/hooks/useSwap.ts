...
const useSwap = () => {
  ...
  const [networkselectedA, setNetworkSelectedA] = useState<any>({
    image: "/images/base.png",
    name: "BASE",
    id: 84532,
    address: "0x601566d18cdaE8D4347bB6ba43C5C2247D9c1f5a",
    CCIP_BnM: "0x88A2d74F47a237a62e7A51cdDa67270CE381555e",
    CCIP_LnM: "0xA98FA8A008371b9408195e52734b1768c0d1Cb5c",
  });
  ...
  const networks = [
    ...
    },
    {
      ...
      name: "OPTIMISM",
      id: 11155420,
      CCIP_BnM: "0x8aF4204e30565DF93352fE8E1De78925F6664dA7",
      CCIP_LnM: "0x044a6B4b561af69D2319A2f4be5Ec327a6975D0a",
      address: "0xb4BF84b079E080Be165174357cEdC10FACAAB9Ae",
    ...
    },
    {
      ...
      name: "CELO",
      id: 44787,
      address: "0xa2EF6cCB0b4A6c23FBb4e56e839456613bF03970",
      CCIP_BnM: "0x7e503dd1dAF90117A1b79953321043d9E6815C72",
      CCIP_LnM: "0x7F4e739D40E58BBd59dAD388171d18e37B26326f",
    ...
    },
    {
      ...
      name: "MODE",
      id: 919,
      address: "0x2Db56C7de28B1B78b623715c98f74156790f82c8",
      CCIP_BnM: "0xB9d4e1141E67ECFedC8A8139b5229b7FF2BF16F5",
      CCIP_LnM: "0x86f9Eed8EAD1534D87d23FbAB247D764fC725D49",
  ...
  const getSigner = () => {
    try {
      if (provider) {
        ...
        let _network = cryptoSupportData.filter((item) => {
          return item.id == _chainId;
        });
        let _networkSelected = networks.filter((item) => {
          return item.id == _chainId;
        });
  ...
  const selectToken = (_tokenA: any, _tokenB: any) => {
    if (Object.keys(_tokenA).length > 0 && Object.keys(_tokenB).length > 0) {
      ...
      if (_tokenA.address === _tokenB.address) {
        if (
          _tokenA.address ===
            SuppotedWrappedETHContractAddress(provider?._network.chainId) &&
          _tokenA.symbol !== _tokenB.symbol
        ) {
          ...
          resetToken();
          toast.error(
            "The selected tokens are identical, please select another token!",
          );
          return;
        }
      } else {
        ...
        if (_paths.length <= 0) {
          resetToken();
          toast.error(
            `There is no swap path from ${_tokenA.symbol} to ${_tokenB.symbol}!`,
          );
          return;
        }
  ...
  const initGraph = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < nPairs; i++) {
        ...
        let tokenPair = new ethers.Contract(
          pairAddress,
          TokenPairABI,
          provider,
        );
  ...
  const checkAllowance = useCallback(async () => {
    ...
    try {
      ...
      let _allow = await _token.allowance(
        address,
  ...
  const getSpendingAmount = async () => {
    ...
    try {
      ...
      for (const path of paths) {
        ...
        const _amountA = Number(
          ethers.utils.formatUnits(amounts[0], tokenA.decimals),
        );
        if (_amountA < min) {
          min = _amountA;
          _bestPath = path;
  ...
  const estimatePriceImpact = async (
    ammRouter: any,
    path: any,
    newPrice: any,
  ) => {
    ...
    for (let i = 0; i < path.length - 1; i++) {
      const [reserveA, reserveB] = await ammRouter.getReserves(
        path[i],
        path[i + 1],
      );
      //@ts-ignore
  ...
  const handleApprove = async () => {
    ...
    try {
      const _token = new ethers.Contract(tokenA.address, ERC20ABI, signer);
      const _allowAmount = ethers.utils.parseUnits(
        toString(amountA),
        tokenA.decimals,
      );
  ...
  const handleWrap = async () => {
    ...
    try {
      ...
      await tx.wait();
      toast.info(
        `${swapMode === MODE_WRAP ? "wrap" : "unwrap"} succeeded! Transaction Hash: ${tx.hash}`,
      );
      setAmountA(0);
      setAmountB(0);
      await getBalances();
  ...
  const getSupportedTokens = useCallback(
    async (erc20Only?: any, customTokens?: any) => {
